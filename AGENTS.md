...
- **`pool_size` upper bound is provider-specific**: `config.Validate` receives `maxConcurrent` from `VMProvider.Capabilities().MaxConcurrent`. Each provider implementation must document why its limit is what it is (see `provider.TartProvider.Capabilities`). The error message names the provider so operators know which constraint applies.
